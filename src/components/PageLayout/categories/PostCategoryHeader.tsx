...
export default function PostCategoryHeader({
  totalCount,
  filteredCount,
  searchQuery,
  onSearchChange,
  ...
  return (
    <div className="p-5 border-b border-gray-200 bg-white">
      <div className="flex items-center justify-between gap-6">
        <div>
          <h2 className="text-lg font-semibold text-gray-900">Danh sách Danh mục bài viết</h2>
        </div>
...
export function PostCategoryTable({ categories, loading, onEdit, onDelete }: PostCategoryTableProps) {
  const columns = [
    { key: 'id', label: 'ID', width: '60px', sortable: true, className: 'text-center' },
    { key: 'name', label: 'TÊN DANH MỤC', sortable: true, className: 'text-left font-medium', width: '200px' },
    { key: 'slug', label: 'SLUG', width: '200px', sortable: true, className: 'text-center' },
    { key: 'description', label: 'MÔ TẢ', width: '250px', sortable: true, className: 'text-left' },
    { key: 'parent_id', label: 'PARENT ID', width: '100px', sortable: true, render: (value: string | null) => value || 'None', className: 'text-center' },
    { key: 'created_at', label: 'NGÀY TẠO', width: '150px', sortable: true, className: 'text-center' },
    { key: 'updated_at', label: 'NGÀY CẬP NHẬT', width: '150px', sortable: true, className: 'text-center' },
    {
      key: 'actions',
      label: 'THAO TÁC',
      width: '120px',
      sortable: false,
      render: (_value: any, row: PostCategory) => (
  ...
  return (
    <CustomTable
      ...
      searchable={false}
      actionable={false}
      emptyText="Chưa có danh mục bài viết nào"
    />
  );
}